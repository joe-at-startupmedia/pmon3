...
func (suite *Pmon3DependencyTestSuite) assertProcessOrderFromCmdResp(processNames []string, processList []*protos.Process) bool {

	passing := assert.Equal(suite.T(), len(processNames), len(processList))
	if !passing {
		return false
	}
