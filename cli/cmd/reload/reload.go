...
func cmdRun(args []string) {
	...
	uid := os.Geteuid()
	if uid != 0 {
		pmond.Log.Fatal("command need root or with sudo permission!")
	}

	// check flag
	if len(sigFlag) > 0 {
		if !array.In(signals, strings.ToUpper(sigFlag)) {
			pmond.Log.Error("the signal only support: HUP，USR1，USR2")
			return
		}
	}
	...
	err = pmond.Db().First(&process, "id = ? or name = ?", processVal, processVal).Error
	if err != nil {
		pmond.Log.Fatalf("process not found: %s", processVal)
	}

	// 验证进程id的状态
	_, err = os.Stat(fmt.Sprintf("/proc/%d/status", process.Pid))
	if os.IsNotExist(err) {
		pmond.Log.Errorf("the process %s pid %d not running \n", process.Name, process.Pid)
		return
	}
	...
	if err = pmond.Db().Save(&process).Error; err != nil {
		pmond.Log.Fatalf("try to set process %s state err: %s", processVal, err.Error())
	}
	...
	if err == nil { // process exist
		...
		pmond.Log.Debugf("send signal to process: %s", getSignal().String())
		if err != nil {
			pmond.Log.Fatalf("try send signal to process err: %s", err)
		}
	}
	...
	newPid := <-pidChannel
	if newPid > 0 { // 进程重启成功
		pmond.Log.Debugf("reload success process id: %d", newPid)
		process.Pid = newPid
		process.Status = model.StatusRunning
		if err = pmond.Db().Save(&process).Error; err != nil {
			pmond.Log.Fatalf("try to update process %s state err %s", processVal, err.Error())
		}
		output.TableOne(process.RenderTable())
	} else { // 重启失败，db数据还原
		pmond.Log.Debugf("reload failed, roll process status: %s", oldState)
		process.Status = model.StatusFailed
		if err = pmond.Db().Save(&process).Error; err != nil {
			pmond.Log.Fatalf("reloadl failed, try rollback process err: %s", err.Error())
		}
		pmond.Log.Fatalf("process %s reload failed", processVal)
	}
}
